...
This is not reversible. Your account and all related data will be deleted permanently.
Later, you can create a new account using the old email address.
...
Yes, FastCron accepts payments via Binance.
Please contact me at support@fastcron.com for the Binance pay ID.
The payment will be processed manually.

## Cron jobs
### Do you support HTTP and HTTPS?
Yes, FastCron support both HTTP and HTTPS protocol.
...
You can also add username and password directly to your cronjob URL like this:
```
...
### Does FastCron follows redirections?
Yes, FastCron will follows redirections (301, 302, 307, 308).

This feature helps in these cases:
- You changed the script URL with redirections.
- The cron script needs to split to several steps (e.g. Akeeba backup).

### What HTTP methods are supported?
We support GET, POST, PUT, PATCH, HEAD, OPTIONS, DELETE, TRACE, and CONNECT.
...
```
For JSON payload, FastCron automatically add HTTP header
`Content-Type: application/json`
to the HTTP request, so you don't need to.

### Will my cronjobs overlap?
Yes, similar to your hosting's cron jobs, your script will overlap.

If you set cron job to run every minute, and your script take more than 1 minute to complete, it'll overlap. There will be 2 instances running at the same time.
